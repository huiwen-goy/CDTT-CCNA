...
#### Figure 4. Residuals from SRT-PTA, versus MoCA
```{r, echo=FALSE}

# Regression: SRT ~ PTA
m1 <- lm(SRT ~ PTA.better, data2)

# Extract residuals for three PTA Groups
...
#Warning: "Removed 6 rows containing missing values (geom_point)" ...but no NA values?
#Note!!! Jittering x-values meant any values over the strict x-limits were not plotted

```

#### Figure 4b. Residuals from SRT-PTA versus MocA, with regression lines for PTA Groups
...
# Regressions for three PTA Groups
...
ggplot(data2, aes(y = m1$residuals, x = jitter(MoCA.total), colour = PTA.group, shape = PTA.group)) + 
  ...
  annotate(geom = "text", x=20, y=-1.75, label = "Better SRT than expected", size = 3) +
  geom_hline(yintercept = 0, lty = "dashed", color = "black") + 
  geom_segment(x = 18.9, xend = 30.1, y = 0.4020779, yend = 0.1468081, colour="seagreen3", size=0.5) + 
  geom_segment(x = 17.8, xend = 30.2, y = -0.5230071, yend = -0.8522137, colour="dodgerblue", size=0.5) + 
